...
set.seed(5627)
t <- try(
trained_model <- train(Class ~ ., data = down_train,
                       method=model, metric='ROC',
                       trControl=train_control,
                       tuneLength=30))
if (inherits(t, 'try-error')) {
  model_NULL <- NULL
  saveRDS(model_NULL, paste0('down_', model, '_', assay, '_', correlation, '_NULL.rda'))
  fwrite(data.table(), paste0('down_', model, '_', assay, '_', correlation, '_NULL.tsv'))
  } else {

saveRDS(trained_model, paste0('down_', model, '_', assay, '_', correlation, '.rda'))
...
write.table(statistics_df, paste0('down_', model, '_', assay, '_', correlation, '.tsv'), row.names=F, col.names=T, sep='\t', quote=F)}
