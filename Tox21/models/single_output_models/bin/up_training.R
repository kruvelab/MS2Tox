...
set.seed(9560)
up_train <- upSample(x=data[, -1], y=data$Class)
...
set.seed(5627)
t <- try(
trained_model <- train(Class ~ ., data = up_train,
...
if (inherits(t, 'try-error')) {
  model_NULL <- NULL
  saveRDS(model_NULL, paste0('up_', model, '_', assay, '_', correlation, '_NULL.rda'))
  fwrite(data.table(), paste0('up_', model, '_', assay, '_', correlation, '_NULL.tsv'))
  } else {

saveRDS(trained_model, paste0('up_', model, '_', assay, '_', correlation, '.rda'))
...
write.table(statistics_df, paste0('up_', model, '_', assay, '_', correlation, '.tsv'), row.names=F, col.names=T, sep='\t', quote=F)}
