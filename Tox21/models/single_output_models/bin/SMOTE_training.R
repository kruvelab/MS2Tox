...
set.seed(9560)
smote_train <- smote(Class ~., data=data, perc.over=((2000/sum(data$Class =='X1'))-1), perc.under=(2000/(1824-sum(data$Class =='X1'))))
smote_train$Class <- relevel(smote_train$Class, ref='X1')
...
set.seed(5627)
t <- try(
trained_model <- train(Class ~ ., data = smote_train,
                       method=model, metric='ROC',
                       trControl=train_control,
                       tuneLength=30))
if (inherits(t, 'try-error')) {
  model_NULL <- NULL
  saveRDS(model_NULL, paste0('smote_', model, '_', assay, '_', correlation, '_NULL.rda'))
  fwrite(data.table(), paste0('smote_', model, '_', assay, '_', correlation, '_NULL.tsv'))
  } else {

saveRDS(trained_model, paste0('smote_', model, '_', assay, '_', correlation, '.rda'))
...
write.table(statistics_df, paste0('smote_', model, '_', assay, '_', correlation, '.tsv'), row.names=F, col.names=T, sep='\t', quote=F)}
