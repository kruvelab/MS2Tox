...
```{r}
validation_set <- as.data.frame(fread('./data/validation_set_final.tsv', sep='\t', header=T))
...
write.table(validation_set_final_groups, './data/validation_set_with_groupIDs.tsv', sep='\t', col.names=F, row.names=F, quote=F)
...
write.table(validation_set_fg_orbitrap, './data/validation_set_orbitrap.tsv', sep='\t', col.names=F, row.names=F, quote=F)
...
write.table(validation_set_fg_qtof, './data/validation_set_qtof.tsv', sep='\t', col.names=F, row.names=F, quote=F)
...
```{r}
validation_set_SIRIUS <- fread('./data/validation_set_SIRIUS.tsv', sep='\t', header=F)
...
#write.table(real_life_test_set_ready, './data/real_life_test_set_ready.tsv', sep='\t', col.names=T, row.names=F, quote=F)
real_life_test_set_ready <- fread('./data/real_life_test_set_ready.tsv', sep='\t',header=T)
fps_for_check <- fread('./data/validation_set_fps.tsv', sep='\t', header=T)

train09 <- as.data.frame(fread('./data/train_filtered_09.tsv', header=T, sep='\t'))
