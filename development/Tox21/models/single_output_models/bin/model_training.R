...
set.seed(123)
equal_train_10 <- anticlustering(data[, 1], K=10, objective="diversity", standardize=F)

indices_in = list(which(equal_train_10 != 1), which(equal_train_10 != 2), which(equal_train_10 != 3), 
                  which(equal_train_10 != 4), which(equal_train_10 != 5), which(equal_train_10 != 6), 
                  which(equal_train_10 != 7),which(equal_train_10 != 8), which(equal_train_10 != 9), 
                  which(equal_train_10 != 10)) 
indices_out = list(which(equal_train_10 == 1), which(equal_train_10 == 2), which(equal_train_10 == 3), 
                   which(equal_train_10 == 4), which(equal_train_10 == 5), which(equal_train_10 == 6), 
                   which(equal_train_10 == 7),which(equal_train_10 == 8), which(equal_train_10 == 9), 
                   which(equal_train_10 == 10))
...
train_control <- trainControl(method='cv', number=10, savePredictions=T, classProbs=T,
                              summaryFunction=twoClassSummary, index=indices_in, indexOut=indices_out)

unlink(".RData")
t <- try(
trained_model <- train(Class ~ ., data=data,
                       method=model,
                       metric='ROC',
...
if (inherits(t, 'try-error')) {
  model_NULL <- NULL
  saveRDS(model_NULL, paste0(model, '_', assay, '_', correlation, '_NULL.rda'))
  fwrite(data.table(), paste0(model, '_', assay, '_', correlation, '_NULL.tsv'))
  } else {

saveRDS(trained_model, paste0(model, '_', assay, '_', correlation, '.rda'))
...
write.table(statistics_df, paste0(model, '_', assay, '_', correlation, '.tsv'), row.names=F, col.names=T, sep='\t', quote=F)}
